...
void insert(struct No **cabeca, void *valor, Token token, size_t tamanho)
{
...
void print_list(struct No *head)
{
...
void identificador(char *texto, struct No **cabeca)
{
  ...
  {
    insert(cabeca, (void *)&numero, Numero, tamanhoNumero);
  ...
  {
  case '+':
    insert(cabeca, texto, Mais, sizeof(*texto));
    break;
  case '-':
    insert(cabeca, texto, Menos, sizeof(*texto));
    break;
  case '*':
    if (*(texto + 1) == '*')
    {
      insert(cabeca, texto, Exponenciacao, sizeof(*texto) + 1);
      espacoCaracter = 2;
    }
    else
    {
      insert(cabeca, texto, Multiplicacao, sizeof(*texto));
    }
    break;
  case '/':
    insert(cabeca, texto, Divisao, sizeof(*texto));
    break;
  default:
    if (!isspace(*texto))
    {
      insert(cabeca, texto, Indeterminado, sizeof(*texto));
    }
    break;
  }
